...
pub fn handle_error(err: Box<dyn std::error::Error + Send + Sync>) -> (StatusCode, Json<ErrorResponse>) {
    error!("Unhandled error: {}", err);
    
    let (status_code, message) = match err.downcast_ref::<crate::error::AppError>() {
        Some(app_error) => {
            match app_error {
                crate::error::AppError::Config(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Configuration error"),
                crate::error::AppError::Crypto(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Cryptographic error"),
                ...
                crate::error::AppError::Internal(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Internal error"),
            }
        }
