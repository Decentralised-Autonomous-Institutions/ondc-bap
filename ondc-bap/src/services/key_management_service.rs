...
use ondc_crypto_traits::{ONDCCryptoError, KeyPair, Signer};
...
impl KeyManagementService {
    ...
    /// Load keys from configuration
    #[instrument(skip(config))]
    async fn load_keys_from_config(config: &KeyConfig) -> Result<KeyPairWithMetadata, KeyManagementError> {
    ...
    async fn load_signing_key(key_b64: &str) -> Result<Ed25519Signer, KeyManagementError> {
        let key_bytes = decode_signature(key_b64)
            .map_err(|e| KeyManagementError::ValidationFailed(format!("Invalid signing key format: {}", e)))?;
        
        if key_bytes.len() != 32 {
            return Err(KeyManagementError::ValidationFailed(
                format!("Invalid signing key length: expected 32 bytes, got {}", key_bytes.len())
            ));
        }
        
        let signer = Ed25519Signer::new(&key_bytes)
    ...
    async fn load_encryption_key(key_b64: &str) -> Result<X25519KeyExchange, KeyManagementError> {
        let key_bytes = decode_signature(key_b64)
            .map_err(|e| KeyManagementError::ValidationFailed(format!("Invalid encryption key format: {}", e)))?;
        
        if key_bytes.len() != 32 {
            return Err(KeyManagementError::ValidationFailed(
                format!("Invalid encryption key length: expected 32 bytes, got {}", key_bytes.len())
            ));
        }
        
        let key_exchange = X25519KeyExchange::new(&key_bytes)
            .map_err(|e| KeyManagementError::Crypto(e))?;
    ...
    pub async fn get_signing_key(&self) -> Result<Ed25519Signer, KeyManagementError> {
        ...
        if let Some(ref signer) = keys.signing_key {
            ...
            Err(KeyManagementError::KeyNotFound("Signing key not available".to_string()))
        }
    }
    ...
    pub async fn get_encryption_key(&self) -> Result<X25519KeyExchange, KeyManagementError> {
        ...
        if let Some(ref key_exchange) = keys.encryption_key {
            ...
            Err(KeyManagementError::KeyNotFound("Encryption key not available".to_string()))
        }
    }
    ...
    pub async fn validate_key_pairs(&self) -> Result<(), KeyManagementError> {
        ...
        let _signature = signer.sign(test_message)
            .map_err(|e| KeyManagementError::Crypto(e))?;
    ...
    pub async fn rotate_keys(&self) -> Result<(), KeyManagementError> {
        ...
        let new_signing_key = Ed25519Signer::generate()
            .map_err(|e| KeyManagementError::Crypto(e))?;
        let new_encryption_key = X25519KeyExchange::generate()
            .map_err(|e| KeyManagementError::Crypto(e))?;
    ...
    pub async fn export_keys(&self, format: KeyFormat) -> Result<ExportedKeys, KeyManagementError> {
        ...
        let exported_keys = match format {
            ...
            KeyFormat::Raw => {
                return Err(KeyManagementError::UnsupportedFormat("Raw format not supported for export".to_string()));
            }
        };
