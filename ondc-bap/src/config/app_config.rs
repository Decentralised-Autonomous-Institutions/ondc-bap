...
impl BAPConfig {
    ...
    pub fn load() -> Result<Self, ConfigError> {
        ...
        if !std::path::Path::new(&config_path).exists() {
            return Err(ConfigError::LoadError(format!("Config file not found: {}", config_path)));
        }
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.ondc.subscriber_id.is_empty() {
            return Err(ConfigError::InvalidSubscriberId("Subscriber ID cannot be empty".to_string()));
        }
...
impl KeyConfig {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        let signing_key = decode_signature(&self.signing_private_key)
            .map_err(|_| ConfigError::InvalidSigningKey("Invalid signing key format".to_string()))?;
        ...
        let encryption_key = decode_signature(&self.encryption_private_key)
            .map_err(|_| ConfigError::InvalidEncryptionKey("Invalid encryption key format".to_string()))?;
        if encryption_key.len() != 32 {
            return Err(ConfigError::InvalidEncryptionKeyLength(encryption_key.len()));
        }
